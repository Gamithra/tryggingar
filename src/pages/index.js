...
const DepositCalculator = () => {
  ...
  const fetchCurrentRates = async () => {
    ...
    try {
      ...
      setError(common.loadRateDataError);
    ...
    <I18nProvider locale={LOCALE}>
      <SiteLayout
        ...
        subtitle={
          <>
            {t.subtitleIntro}{' '}
            <span className="brutal-marker">{t.subtitleHighlight}</span>{' '}
            {t.subtitleOutro}
          </>
        }
          ...
          <>
            {rateMeta?.fetchedAt && (
              <p className="text-sm text-[var(--color-text-muted)] brutal-mono">
                {t.dataFetched}: {formatFetchedAt(rateMeta.fetchedAt, LOCALE)}
                {' · '}
      ...
      >
              ...
              <div className="space-y-6">
                ...
                <div className="brutal-card-inset overflow-hidden">
                  ...
                  <dl className="p-4 space-y-3 text-sm">
                    {[
                      [t.originalDeposit, formatCurrency(results.principal)],
                      [t.duration, `${results.daysDiff} ${t.days}`],
                      [t.grossInterest, formatCurrency(results.grossInterest)],
                      [
                        `${t.capitalGainsTax} (${CAPITAL_GAINS_TAX}%)`,
                    ...
                    <div className="flex justify-between gap-4 text-xs">
                      <dt className="text-[var(--color-text-muted)]">{t.effectiveAnnualRate}</dt>
        ...
        <section className="mt-10 md:mt-12 brutal-card p-6 md:p-8">
          <h2 className="text-xl font-black uppercase tracking-tight mb-6">{t.faqHeading}</h2>
